...
class DefaultController extends Controller
{
    ...
    public function clientAction()
    {
        ...
        return $this->render('base.html.twig',array('page'=>'client','titre' =>'Projet NewsLetter','soustitre'=>'Édition des clients','clients'=>$clients, 'groupes' => $groupe ));
    }
...
public function CampagneSendAction($campagneId)
{
    ...
    foreach ($clientsList as $clt) {
      foreach ($newslettersList as $nll) {
        ...
        $bodymail = fopen($nll->getHtmlPath());
        $message = \Swift_Message::newInstance()
        ->setSubject('NewsLetter Projet Arias Carré Bréda')
            ->setFrom('no-reply@cochise-corp.fr') // notre mail
            ->setTo($clt->getEmail())
            ->setReadReceiptTo('tom.breda@live.fr')
            ->setBody($bodymail)
...
public function addNLToCampagneAction()
{
    ...
    $repositoryCampagne = $this->getDoctrine()->getManager()->getRepository('PNLindexBundle:Campagne');
    $campagne = $repositoryCampagne->findOneById($campagneId);
    return $this->render('base.html.twig',array('page'=>'campagne','titre' =>'NewsLetter ajouté à la campagne '.$campagne->getNom(),'soustitre'=>'Gaël Arias, Alexandre Carré, Tom Bréda' ));
}
